...
class AsyncRedisCacheClient(BaseRedisCacheClient):
    def __init__(
        self,
        servers,
        serializer=None,
    ...
    async def get_client(self, key=None, *, write=False):
        # key is used so that the method signature remains the same and custom
        # cache client can be implemented which might require the key to select
        # the server, e.g. sharding.
        pool = self._get_connection_pool(write)
        return await self._client(connection_pool=pool)
    ...
    async def close(self, **kwargs):
        client = await self.get_client(None, write=False)
        await client.close(**kwargs)
...
class AsyncRedisCache(BaseCache):
    def __init__(self, server, params):
        ...
        self._class = AsyncRedisCacheClient
        self._options = params.get("OPTIONS", {})
    ...
    async def aadd(self, key, value, timeout=DEFAULT_TIMEOUT, version=None):
        key = self.make_and_validate_key(key, version=version)
        return await self._cache.add(key, value, self.get_backend_timeout(timeout))
    ...
    async def aset(self, key, value, timeout=DEFAULT_TIMEOUT, version=None):
        key = self.make_and_validate_key(key, version=version)
        await self._cache.set(key, value, self.get_backend_timeout(timeout))

    async def atouch(self, key, timeout=DEFAULT_TIMEOUT, version=None):
        key = self.make_and_validate_key(key, version=version)
        return await self._cache.touch(key, self.get_backend_timeout(timeout))
    ...
    async def aget_many(self, keys, version=None):
        ...
        ret = await self._cache.get_many(key_map.keys())
        return {key_map[k]: v for k, v in ret.items()}

    async def ahas_key(self, key, version=None):
        key = self.make_and_validate_key(key, version=version)
        return await self._cache.has_key(key)
    ...
    async def aset_many(self, data, timeout=DEFAULT_TIMEOUT, version=None):
        ...
        for key, value in data.items():
            key = self.make_and_validate_key(key, version=version)
            safe_data[key] = value
        await self._cache.set_many(safe_data, self.get_backend_timeout(timeout))
        return []
    ...
    async def adelete_many(self, keys, version=None):
        if not keys:
            return
        safe_keys = [self.make_and_validate_key(key, version=version) for key in keys]
        await self._cache.delete_many(safe_keys)

    async def aclear(self):
        return await self._cache.clear()

    async def aclose(self, **kwargs):
        return await self._cache.close(**kwargs)
