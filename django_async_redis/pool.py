...
from redis.asyncio.connection import DefaultParser, to_bool
...
class ConnectionFactory:
    ...
    def __init__(self, options):
        pool_cls_path = options.get(
            "CONNECTION_POOL_CLASS", "redis.asyncio.connection.ConnectionPool"
        )
        ...
        redis_client_cls_path = options.get(
            "REDIS_CLIENT_CLASS", "redis.asyncio.client.Redis"
        )
    ...
    async def connect(self, url: str) -> Redis:
        """
    ...
    async def disconnect(self, connection: Redis):
        """
    ...
    async def get_connection(self, params: Dict[str, Any]) -> Redis:
        ...
        return await self.redis_client_cls(
    ...
    def get_or_create_connection_pool(self, params: Dict[str, Any]):
        """
    ...
    def get_connection_pool(self, params: Dict[str, Any]):
        """
...
class SentinelConnectionFactory(ConnectionFactory):
    def __init__(self, options):
        ...
        options.setdefault(
            "CONNECTION_POOL_CLASS", "redis.asyncio.sentinel.SentinelConnectionPool"
        )
